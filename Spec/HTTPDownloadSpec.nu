...
(describe "HTTPDownload" `(
  ...
  (it "calls the global connection delegate when a client side failure occurred" (do ()
    (set @called nil)
    (set download (HTTPDownload downloadFromURL:(NSURL URLWithString:"http://localhost:9292/status-code?code=404") nuBlock:(do (response)
      (set @called t)
    )))
    (wait 0.1 (do ()
      (~ @called should be:nil)
      ;(puts (download errorMessage))
      (~ (download errorMessage) should be:"Response 404, not found.")
  ...
  (it "calls the global connection delegate when an error occurs" (do ()
    ...
    (wait 1.1 (do ()
      ...
      (~ (download errorMessage) should be:"The request timed out.")
      (~ ((@delegate methodCalls) valueForKey:"downloadFailed:") should be:download)
