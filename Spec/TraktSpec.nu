...
(describe "Trakt" `(
  ...
  (describe "shared instance" `(
    ...
    (describe "concerning the user's calendar" `(
      ...
      (it "sorts the episodes on airtime and then by show name" (do ()
        (@trakt calendarWithNuBlock:(do (calendar)
          ...
          (set episode ((date episodes) objectAtIndex:0))
          (~ ((episode show) title) should be:"Episodes")
          (~ (episode title) should be:"Episode Five")

          (set episode ((date episodes) objectAtIndex:1))
          (~ ((episode show) title) should be:"Flashpoint")
          (~ (episode title) should be:"Fault Lines")

          (set episode ((date episodes) objectAtIndex:2))
          (~ ((episode show) title) should be:"Top Gear")
          (~ (episode title) should be:"Series 16, Episode 3")
        ))
        (wait 0.1 (do ()
          ; Nothing... We just wait with further spec execution until the ImageDownload is (probably) finished.
        ))
    ...
    (it "returns the recommendations URL" (do ()
      (~ ((@trakt recommendationsURL) absoluteString) should be:"http://localhost:9292/api/shows/recommendations.json")
    ))

    (it "yields the recommendations as an array of Show instances" (do ()
      (@trakt recommendationsWithNuBlock:(do (recommendations)
        ;(puts recommendations)
        (set show (recommendations objectAtIndex:0))
        (~ show should be kindOfClass:Show)
